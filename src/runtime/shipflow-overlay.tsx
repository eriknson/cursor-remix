...
} from './register-clipboard-interceptor';
...
function useSelectionEvents(
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(EVENT_OPEN, handler as EventListener);
...
function useAutoFocus(
  isOpen: boolean,
  shadowRoot: ShadowRoot | HTMLElement | null,
) {
  useEffect(() => {
...
function useClickOutside(
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const clickedInsideHighlight =
        highlightedElement && path.includes(highlightedElement);
...
function Typewriter({ text }: { text: string }): ReactElement {
...
function Bubble({
  ...
  const [bubbleStyle, setBubbleStyle] =
    useState<CSSProperties>(DEFAULT_BUBBLE_STYLE);
  ...
  useLayoutEffect(() => {
    if (!bubbleSize) {
      setBubbleStyle((prev) =>
        prev === DEFAULT_BUBBLE_STYLE ? prev : DEFAULT_BUBBLE_STYLE,
      );
      return;
    }
    ...
    if (anchor) {
      ...
      const bottomTop =
        anchorViewport.top + anchorViewport.height + verticalGap;
      ...
      const rightLeft =
        anchorViewport.left + anchorViewport.width + horizontalGap;
    ...
    if (!bestStyle && pointer) {
      ...
      const fitsRight =
        rightLeftRaw + bubbleSize.width <= viewportWidth - VIEWPORT_MARGIN;

      let targetLeft = rightLeftRaw;
      if (!fitsRight) {
        const leftLeftRaw =
          pointer.x - POINTER_HORIZONTAL_GAP - bubbleSize.width;
  ...
  const computedStatusLabel =
    chat.statusLabel ??
    statusSequence[chat.statusPhase] ??
    statusSequence[0] ??
    null;
  ...
  return (
    ...
    >
      ...
      {chat.statusAddonMode !== 'idle' && (
        <div data-sf-status="true" data-mode={chat.statusAddonMode}>
          {chat.statusAddonMode === 'progress' ? (
            <div data-sf-status-header="true">
              <div data-sf-status-label="true">
                <CursorIcon loading />
                <span data-sf-shimmer="true">{computedStatusLabel}</span>
              </div>
              {chat.statusContext && (
                <span data-sf-status-context="true">
                  {chat.useTypewriter ? (
                    <Typewriter text={chat.statusContext} />
                  ) : (
                    chat.statusContext
                  )}
                </span>
      ...
      {chat.error ? <div data-sf-error="true">{chat.error}</div> : null}
    </div>
  );
}
...
export function ShipflowOverlay(props: ShipflowOverlayProps = {}) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (
        mount.container.childNodes.length === 0 &&
  ...
  useEffect(() => {
    ...
    loadReactGrabRuntime({
      ...
      .catch((error) => {
        console.error(
          '[shipflow-overlay] Failed to load React Grab runtime',
          error,
        );
      });
  ...
  const sendToBackend = useCallback(
    async (payload: {
      ...
      const promotePhase = (phase: number) => {
        const safePhase = Math.min(
          Math.max(phase, 0),
          config.statusSequence.length - 1,
        );
        setChat((prev) => {
          if (!prev) return prev;
          ...
          return {
            ...
            statusLabel:
              config.statusSequence[safePhase] ?? fallbackStatusLabel,
          };
        });
      };
      ...
      try {
        ...
        if (!reader) {
          throw new Error(
            'Streaming response is not supported in this environment.',
          );
        }
        ...
        const processEvent = (event: StreamEvent) => {
          if (event.event === 'status') {
            ...
            if (message) {
              ...
              if (
                !hasPromotedUpdating &&
                /apply|build|final|update/i.test(message)
              ) {
                promotePhase(2);
              ...
              setChat((prev) =>
                prev
                  ? {
                      ...
                      statusLabel: null,
                      statusContext: null,
                      statusPhase: Math.max(
                        config.statusSequence.length - 1,
                        0,
                      ),
                      serverMessage: event.stderr
                        ? event.stderr.trim()
                        : prev.serverMessage,
                    }
                  : prev,
          ...
          while (newlineIndex !== -1) {
            ...
            if (line) {
              try {
                ...
                console.warn('[shipflow-overlay] Unable to parse stream line', {
                  line,
                  error,
                });
              }
        ...
        if (finalLine) {
          try {
            ...
            console.warn(
              '[shipflow-overlay] Unable to parse final stream line',
              { finalLine, error },
            );
          }
        }
        ...
        console.error(
          '[shipflow-overlay] Failed to communicate with Cursor CLI backend',
          error,
        );
        setChat((prev) =>
          prev
